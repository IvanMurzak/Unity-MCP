...
export const UNITY_EDITOR_BASE_DIRS = {
  win32: [
...
export const UNITY_EXECUTABLE_SUBPATH = {
  win32: 'Editor/Unity.exe',
  darwin: 'Unity.app/Contents/MacOS/Unity',
  linux: 'Editor/Unity',
};

/** Path to the connect-result file within a Unity project's Library folder. */
export const CONNECT_RESULT_RELATIVE_PATH = path.join('Library', 'mcp-connect-result.json');

/**
 * Returns the absolute path to the mcp-connect-result.json file.
 * @param {string} projectPath
 * @returns {string}
 */
export function getConnectResultPath(projectPath) {
  return path.join(projectPath, CONNECT_RESULT_RELATIVE_PATH);
}
 ...
 */
export function findUnityExecutable(version) {
  const platform = process.platform;
 ...
 * @param {boolean} [options.detached=true] - Whether to detach the child process (default true)
...
export async function openUnityProject(projectPath, options = {}) {
  const { unityPath, unityVersion, extraArgs = [], detached = true } = options;
...
/**
 * Polls a file path until it exists or a timeout is reached.
 * @param {string} filePath - Path to poll
 * @param {number} [timeoutMs=30000] - Maximum wait time in milliseconds
 * @param {number} [intervalMs=500] - Poll interval in milliseconds
 * @returns {Promise<boolean>} true if the file appeared, false if timed out
 */
export async function pollForFile(filePath, timeoutMs = 30000, intervalMs = 500) {
