...
Unity MCP Plugin は起動時に以下の環境変数（およびコマンドライン引数）を読み込み、保存済み設定ファイルの値を上書きします。上書きはディスクに**保存されません** — 現在のセッションにのみ影響します。Unity を起動するたびに、上書きを有効にするには環境変数またはコマンドライン引数を指定する必要があります。
