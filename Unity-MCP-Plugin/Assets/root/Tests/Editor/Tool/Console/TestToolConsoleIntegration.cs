...
namespace com.IvanMurzak.Unity.MCP.Editor.Tests
{
    public class TestToolConsoleIntegration : BaseTest
    {
        ...
        public IEnumerator GetLogs_Validate_LogCount()
        {
            ...
            var testCount = 15;
            var timeout = 10000;
            var startCount = Startup.LogUtils.LogEntries;
            ...
            var frameCount = 0;
            while (Startup.LogUtils.LogEntries < startCount + testCount)
            {
                yield return null;
                frameCount++;
        ...
        public IEnumerator GetLogs_Validate_ConsoleLogRetention()
        {
            ...
            var frameCount = 0;
            while (Startup.LogUtils.LogEntries < testCount)
            {
                yield return null;
                frameCount++;
                Assert.Less(frameCount, timeout, "Timeout waiting for logs to be collected.");
            }
            Assert.AreEqual(testCount, Startup.LogUtils.LogEntries, "Log entries count should include new entries.");

            // Save to file and wait for completion
            var saveTask = Startup.LogUtils.SaveToFile();
            frameCount = 0;
            ...
            Startup.LogUtils.ClearLogs(false);
            Assert.AreEqual(0, Startup.LogUtils.LogEntries, "Log entries should be cleared.");
            Assert.AreEqual(0, Startup.LogUtils.GetAllLogs().Length, "Log entries should be cleared.");

            // Load from file and wait for completion
            var loadTask = Startup.LogUtils.LoadFromFile();
            frameCount = 0;
            ...
            var allLogs = Startup.LogUtils.GetAllLogs();

            Assert.AreEqual(Startup.LogUtils.LogEntries, allLogs.Length, "Loaded log entries count should match the saved entries.");

            // Final assertion
            Assert.AreEqual(testCount, Startup.LogUtils.LogEntries, "LogUtils should have the restored logs in memory.");
