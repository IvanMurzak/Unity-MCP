...
namespace com.IvanMurzak.Unity.MCP.Editor.API
{
    public partial class Tool_Tool
    {
        ...
        )
        {
            ...
            {
                var properties = tool.InputSchema is JsonObject schemaObj
                    && schemaObj.TryGetPropertyValue(JsonSchema.Properties, out var propertiesNode)
                    ...
                    if (!matches && properties != null)
                    {
                        foreach (var prop in properties)
                        {
                            if (prop.Value is JsonObject propObj)
                            {
                                ...
                                if (regex.IsMatch(prop.Key) || regex.IsMatch(argDesc))
                                {
                                    matches = true;
                                    break;
                                }
